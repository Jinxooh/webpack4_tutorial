  ...
  renderIntentsTable(type) {
    ...
    return (
      <table>
        <thead>
          <tr>
            {['Code', 'Name', 'Description', 'Category', 'Price'].map(item =>
              (<th key={item} style={{ textAlign: 'left' }}>
                {item}
              </th>),
            )}
          </tr>
        ...
        <tbody>
          {new Array(20).fill(undefined).map((item, index) => {
            return (
              <tr key={index}>
                <td><button className="automate-intent-name" style={{ textAlign: 'left', margin: 0 }} onClick={() => this.props.pushRoute(`${defaultRoute}/product/${type}/111}`)}>{`${type} ${index}`}</button>
                </td>
                {['product name', 'product description', 'category1 - sub2', '2,000'].map(item =>
                  (<td key={item}>
                    {item}
                  </td>),
                )}
      ...
      <div className="automate" style={{ marginLeft: 230 }}>
        <div className="top-container">
          <div className="buttons-container">
            <button className="new">New Product</button>
        ...
        <div className="automate-title-section">
          {this.renderIntentsTable('product')}
        </div>
      </div>
    );
  }
